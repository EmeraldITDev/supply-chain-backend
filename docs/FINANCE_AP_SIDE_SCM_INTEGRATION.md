...
| 4 | **Vendors** | **SCM is vendor master (Pattern A).** On each package/delta push, sync `header.vendor` snapshot into Finance AP. Use **`vendor_scm_mappings`** (`scm_vendor_id` → `finance_ap_vendor_id`). **No live SCM vendor API.** Full FA implementation guide: **`docs/FINANCE_AP_VENDOR_SYNC_PATTERN_A.md`**. |
