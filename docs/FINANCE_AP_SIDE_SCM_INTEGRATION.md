...
| 4 | **Vendors** | **SCM is vendor master.** On each push, sync vendor snapshot into Finance AP. Use **`vendor_scm_mappings`** (`scm_vendor_id` → `finance_ap_vendor_id`). |
...
| Section | Content |
|---------|---------|
| **Header** | `scm_transaction_id`, `mrf_id`, `formatted_id`, `scm_po_number` (reference), vendor snapshot, contract type, department, currency, totals |
